...
function estResetSubDivs(){
  ...
  var ncid = Number($('select[name="prop_city"]').find('option:selected').val());
  console.log(cid,ncid);
  if(ncid !== cid){
    defs.tbls.estate_properties.dta[0].prop_city = ncid;
    $('select[name="prop_subdiv"]').data('subdIdx',Number(0)).val(Number(0));
    $('#estCommunityPreviewCont').empty();
    estSetSubDivEdtBtn();
    }
  }
...
function estSetSubDivEdtBtn(){
  ...
  var edtBtn = $('select[name="prop_subdiv"]').parent().find('button.selEditBtn1 i');
  if(subdIdx > 0){$(edtBtn).removeClass('fa-plus').addClass('fa-pencil-square-o');}
  else{$(edtBtn).removeClass('fa-pencil-square-o').addClass('fa-plus');}
  }

function estGetSubDivs(mode=1){
  console.log('estGetSubDivs');
  var selEle = $('select[name="prop_subdiv"]');
  var subdIdx = Number(selEle.find('option:selected').val());
  var edtBtn = $(selEle).parent().find('button.selEditBtn1 i');
  estSetSubDivEdtBtn();
  console.log(Number($(selEle).val()),subdIdx,mode);
  
  
  if(mode == 2 ||subdIdx !== Number(selEle.data('subdIdx'))){
    selEle.data('subdIdx',subdIdx);
    $('#estCommunityPreviewCont').empty().promise().done(function(){
      $.ajax({
        ...
        success: function(ret, textStatus, jqXHR){
          $('#estCommunityPreviewCont').html(ret);
          },
                  ...
                  if(fnct !== null && typeof fnct.name !== 'undefined'){
                    ...
                    switch (fnct.name){
                      ...
                      case 'estUDSubdivision' : 
                        if(Number(ret.newid) > 0){$('select[name="prop_subdiv"]').val(ret.newid).change();}
                        else if(Number(ret.upres) > 0){$('select[name="prop_subdiv"]').data('subdIdx',-1).change();}
                        break;
