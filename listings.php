...
if(!$data = $sql->retrieve($query,true)){
  ...
  foreach($data as $row){
    ...
    if($prop_idx !== $row['prop_idx']){
      $prop_idx = $row['prop_idx'];
      $EST_PROP[$row['prop_idx']] = $row;
      $FPROPS .= ($FPROPS ? "," : "").$row['prop_idx'];
      }
  ...
  $EVTQRY .= " AND event_start >='".$STRDATETODAY."' AND event_stat='1' AND event_type='1' ORDER BY event_start ASC"; //
